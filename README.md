...
个人资料和审计记录存放在本机目录；配对码只保存在本机文件与扩展本地存储（不使用sync）。不使用云同步、遥测或第三方AI接口。HTTP桥接只绑定127.0.0.1，检查扩展Origin、Host和随机配对码。
...
不是“所有网站一键通吃”：跨域iframe、折叠未展开、下一页、新增多段经历、复杂级联地区、特殊上传控件和验证码需要本人操作后再次扫描；控件回读成功不代表服务器已保存。单选/自定义控件覆盖有限，生产网站仍需逐站验证。
...
配对码改存 chrome.storage.local（仅本机，不用sync），并限制为可信扩展上下文；“清除配对”可删除。旧会话配对存在时尝试迁移；如果Edge更新已清空旧会话，首次需再保存一次。不要卸载扩展或清除扩展数据，否则本机配对也会删除。

岗位网页显示“扫描给Codex”和“填写简历”两个入口。在设置页点“在此网站自动显示按钮”，明确授权当前站点后，之后进入同站点会自动显示，不默认请求所有网站权限。

“扫描给Codex”保存本次表单并授权相关资料读取；Codex调用form_context读取最近扫描，包含现有普通字段值和相关已确认资料（密码、验证码排除），不等于点击后自动唤起Codex新一轮对话。可在对话中说“读取刚才扫描的表单并完善计划”。默认profile_catalog仍不提供值。

控件分类：普通文字、原生select、原生单选组、date/month日期、可识别的Ant/Element下拉或日期、file图片附件。隐藏file输入也进入清单。日期没有完整日时不补造；只读日期面板仅选择当前可见且精确匹配的日期，不盲点月份。自定义控件、复杂地区级联、多选组仍需逐站适配，失败留给本人。

附件：在抽屉选择本机文件，再点击授权上传。最大10MB，校验accept，不覆盖已有附件。由原生File/DataTransfer交给网站上传控件，显示“已交给网页”不等于服务器上传成功，需检查网站回执；不读取任意磁盘路径，不自动提交。

...
export function makePlan(snapshot, profile, mappings={}) {
 ...
 const entries=snapshot.fields.map(f=>{
  const row={fieldId:f.id,label:f.label,section:f.section,kind:f.type,oldValue:f.value,required:f.required,action:f.action,accept:f.accept,multiple:f.multiple};
  ...
  if(f.type==='month'&&!/^\d{4}-\d{2}$/.test(value)) return {...row,status:'missing',reason:'需要准确年月'};
  if(['date','date-picker'].includes(f.type)&&!/^\d{4}-\d{2}-\d{2}$/.test(value)) return {...row,status:'missing',reason:'日期精度不足，不补造日'};
...
export function publicSnapshot(s){return {id:s.id,origin:new URL(s.url).origin,fields:s.fields.map(({id,label,type,section,required,maxLength,options,value,action,accept,multiple})=>({id,label,type,section,required,maxLength,action,accept,multiple,hasValue:value!==''&&value!=null,options:options?.map(o=>({label:o.label}))})),limitations:s.limitations};}
